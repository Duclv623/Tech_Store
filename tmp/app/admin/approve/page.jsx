...
export default function AdminApprove() {
    ...
    const fetchStores = async () => {
        try {
            ...
            // Lấy stores có status pending
            const data = await storesAPI.getByStatus('pending');
            setStores(data || []);
        } catch (error) {
    ...
    const handleApprove = async ({ storeId, status }) => {
        try {
            const { storesAPI } = await import('@/lib/api');
            const updatedStore = await storesAPI.updateStatus(storeId, status);
        ...
        <div className="text-slate-500 mb-28">
            ...
            {stores.length ? (
                ...
                <div className="flex items-center justify-center h-80">
                    <h1 className="text-3xl text-slate-400 font-medium">Không có đơn đăng ký nào đang chờ duyệt</h1>
                </div>
            )}
        </div>
    ) : <Loading />
