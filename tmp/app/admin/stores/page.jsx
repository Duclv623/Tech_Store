...
export default function AdminStores() {
    ...
    const toggleIsActive = async (storeId) => {
        try {
            const { storesAPI } = await import('@/lib/api');
            const updatedStore = await storesAPI.toggleActive(storeId);
            // Update local state
    ...
    return !loading ? (
        <div className="text-slate-500 mb-28">
            <h1 className="text-2xl">Danh sách <span className="text-slate-800 font-medium">Cửa hàng</span></h1>
            ...
            {stores.length ? (
                ...
                <div className="flex items-center justify-center h-80">
                    <h1 className="text-3xl text-slate-400 font-medium">Chưa có cửa hàng nào</h1>
                </div>
            )
            }
        </div>
